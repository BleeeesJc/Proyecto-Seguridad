<template>
  <div>
          ...
          <tr v-for="usuario in filteredUsuarios" :key="usuario.idusuario">
            ...
            <td>
              <select
                ...
                v-model="usuario.selectedRolIndex"
              >
                <option
            ...
            <td>
              ...
              >
                <i
                  class="fas"
                  :class="usuario.isEditing ? 'fa-check' : 'fa-edit'"
                ></i>
              </button>
            </td>
          </tr>
    ...
    <FromNewUser @usuario-creado="onUsuarioCreado" />
  </div>
</template>

<script>
import axios from "axios";
import FromNewUser from "@/components/FromNewUser.vue";
...
export default {
  ...
  computed: {
    filteredUsuarios() {
      const query = this.removeDiacritics(
  ...
  methods: {
    onUsuarioCreado(nuevoUsuario) {
      this.usuarios.unshift({
        ...nuevoUsuario,
        isEditing: false,
        selectedRolIndex: this.roles.findIndex(
          (r) => r.idrol === nuevoUsuario.idrol
        ),
      });
    },
    ...
    async actualizarRol(usuario) {
      try {
        ...
        const response = await axios.put(
          `http://localhost:5000/api/usuario/${usuario.idusuario}/rol`,
          {
            idRol: nuevoRol,
          }
        );
